...
def plot_predictions_zoom(y_true, predictions_dict, title, start_sample, end_sample):
    ...
    time_values = sample_indices * 0.22
...
def main():
    try:
        ...
        # 加载模型
        rf_model = joblib.load("Random_Forest_model_Circuit.pkl")
        scaler_rf = joblib.load("Scaler_RF_Circuit.pkl")
        rf_attention_model = MLPWithAttention(input_dim=rf_model.n_estimators).to(device)
        rf_attention_model.load_state_dict(torch.load("RF_attention_model_Circuit.pth"));
        rf_attention_model.eval()

        scaler_no_rf = joblib.load("Scaler_no_RF_Circuit.pkl")
        no_rf_no_attention_model = MLPWithoutAttention(input_dim=2).to(device)
        no_rf_no_attention_model.load_state_dict(torch.load("No_RF_no_attention_model_Circuit.pth"));
        ...
        lstm_model = LSTMModel(input_size=2, **{k: v for k, v in lstm_config.items() if k != 'SEQUENCE_LENGTH'})
        lstm_model.load_state_dict(torch.load("Circuit_board_lstm_model.pth"));
    ...
    test_datasets = {
        ...
        "15℃": [{"file_path": "C:/Users/hs/Desktop/Sensors_battery.xlsx",
                 "ranges": [("Sheet1", "BN47:BN383", "BO47:BO383")]}],
        "25℃": [{"file_path": "C:/Users/hs/Desktop/Sensors_battery.xlsx",
                       "ranges": [("Sheet1", "O41:O371", "P41:P371")]}],
        "30℃": [{"file_path": "C:/Users/hs/Desktop/Sensors_battery.xlsx",
                            "ranges": [("Sheet1", "Y50:Y386", "Z50:Z386")]}],
    }
    # 循环处理每个测试集
    results_all = {}
