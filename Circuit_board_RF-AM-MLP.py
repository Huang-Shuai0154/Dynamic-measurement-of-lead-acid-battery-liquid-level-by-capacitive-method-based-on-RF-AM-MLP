...
class SimpleKalman1D:
    ...
    v_k ~ N(0, R)           （测量噪声方差）
    """
    def __init__(self, process_var=1e-6, meas_var=1e-3, x0=0.0):
        self.Q = process_var         # 过程噪声协方差
        self.R = meas_var            # 测量噪声协方差
        self.x = x0                  # 先验状态
        self.P = 1.0                 # 先验协方差
    ...
    def smooth(self, z_seq):
        return np.array([self.update(z) for z in z_seq])
# ------------------------------------------------------------------------
...
# data_sources = [
#     {
#         "file_path": "C:/Users/hs/Desktop/battery_level.xlsx",
#         "ranges": [
#             # ("Sheet4","B18:B98","F18:F98"),
...
for source in data_sources:
    ...
    for sheet_name, c_range, h_range in source["ranges"]:
        ...
        # C_raw = read_excel_range(file_path, sheet_name, c_range).flatten()
        #
        # kf = SimpleKalman1D(process_var=1,  # ← 可调
        #                     meas_var=0,  # ← 可调
        #                     x0=C_raw[0])
        # C = kf.smooth(C_raw)  # 用滤波后的 C 继续后面步骤
...
X_train, X_test, y_train, y_test = train_test_split(
    X, y, test_size=0.00001, random_state=42
)
...
train_model(model, X_train_tensor, y_train_tensor, epochs=500)
...
# validation_data_100 = [
#     {
#         "file_path": "C:/Users/hs/Desktop/battery_level.xlsx",#You can put the file to your own desktop and change the path to your desktop accordingly
#         "ranges": [
#             ("Sheet7","BB80:BB380","BC80:BC380"),
#         ]
#     }
# ]
# validation_data_60 = [
...
#         "file_path": "C:/Users/hs/Desktop/battery_level.xlsx",
#         "ranges": [
#             # ("Sheet2", "J176:J603", "L176:L603"),
#             ("Sheet7","CR83:CR436", "CS83:CS436"),
#         ]
#     }
# ]
# validation_data_30 = [
...
def process_validation_data(data_sources, rf_model, scaler, model, process_var=1, measurement_var=0.001, plot_title="Validation"):
    ...
    X_validation = np.hstack((all_avg_C, all_delta_C_window))  # 现在是电容平均值、变化量、振幅和频率作为特征
    y_validation = all_h.flatten()

    # # ===== 新增：用训练阶段的 iqr_bounds 过滤验证集 =====
    # global iqr_bounds
    # X_validation, y_validation = apply_iqr_filter(X_validation,
    #                                               y_validation,
    #                                               iqr_bounds)
    # print(f"{plot_title}: 剔除异常后剩余 {len(y_validation)} 条样本")
    ...
    with torch.no_grad():
        try:
            validation_predictions, _ = model(X_validation_tensor)
            validation_predictions = validation_predictions.squeeze()
        except Exception as e:
...
torch.save(model.state_dict(), "RF_attention_model_Circuit.pth")
joblib.dump(rf_model, "Random_Forest_model_Circuit.pkl")
joblib.dump(scaler_rf, "Scaler_RF_Circuit.pkl")